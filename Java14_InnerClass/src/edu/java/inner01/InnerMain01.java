 ...
 *                             객체를 생성한 후에, 그 참조변수로 접근해서 사용하는 변수.
 ...
 * 1. 멤버 내부 클래스: 필드를 선언하는 위치에서 선언하는 클래스. 접근 수식어 사용 가능.
...
public class InnerMain01 {
	...
	private int x; // 인스턴스 변수 -> 메모리 Heap 영역(생성자를 호출했을 때 생성됨)

	public static void main(String[] args) {
		System.out.println("MAX = " + MAX); // InnerMain01.MAX
		
		InnerMain01 app = new InnerMain01(); // 생성자를 호출 -> 인스턴스 필드가 메모리에 생성 
		System.out.println("x = " + app.x); // app: 참조변수, 생성된 객체를 찾아갈 수 있는 변수
		
//		private int y; -> 지역변수는 접근 수식어 사용 불가능
//		static int y; -> static 선언 불가능.
		
		// 외부 클래스 Outer 타입의 객체를 생성
		Outer outer1 = new Outer(1, "abc", 5);
		System.out.println(outer1);
		
		// 내부 클래스 Inner 타입의 객체를 생성
		// 인스턴스 내부 클래스는 참조 변수를 사용해야 생성자를 호출할 수 있음.
		Inner inner1 = outer1.new Inner(30); // -> import 문장을 사용한 경우
//		Outer.Inner inner1 = outer1.new Inner(2);  -> import문 없는 경우
		inner1.printInfo();
		
		Outer.Inner inner2 = outer1.new Inner(3);
		inner2.printInfo();
		
		System.out.println(Enclosing.var); // -> static 멤버 변수
		Enclosing outer2 = new Enclosing(6);
		System.out.println(outer2);
		
		Nested nested = new Nested(8); // -> import문 있을 때
//		Enclosing.Nested nested = new Enclosing.Nested(8); -> import문 없을 때
		nested.printInfo();
