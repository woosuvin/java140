package edu.java.class02; // 패키지선언문

// 클래스(class): 프로그램에서 필요한 데이터와 기능을 묶어서 하나의 "데이터 타입"으로 정의한 것.
// - 데이터 -> 필드(field), 멤버 변수, 속성(property)
// - 기능 -> 메서드(method)
// 데이터 타입 -> 변수 선언, parameter 선언, 리턴 타입 선언.
// 클래스 선언: [수식어, 생략 가능] class 클래스 이름 { ... }
// 클래스 이름 작성: 
// - 영문자, 숫자, - , $를 사용할 수 있음.
// - 숫자로 시작할 수 없음.
...
public class BasicTv { // 클래스 선언
	// field (멤버변수) - 클래스의 모든 메서드에서 사용 가능.
	...
	public void channelUp() {
		if (powerOn) { // TV 전원이 ON(true)인 경우
			if (channel < MAX) {	// TV 채널 번호가 최댓값보다 작은 경우
				channel++; // 현재 채널 번호를 1 증가.
			} else { // TV 채널 번호가 최댓값보다 크거나 같은 경우
				channel = MIN; // 채널의 최솟값으로 바꿈(채널 순환시킴).
			}
		}
		System.out.println("채널: " + channel); // 출력문을 if문 바깥에
	}
		
	public void channelDown() {
		if (powerOn) { // TV 전원이 ON(true)인 경우
			if (channel > MIN) { // 현채 채널이 0보다 클 때
				channel--; // 현재 채널 번호를 1 감소.
			} else { // 채널이 0보다 작거나 같을 때
	...
	public void volumeUp() {
		if (powerOn) { // if (powerOn && volume < 5)
			if (volume < MAX) {
				volume++; // 현재 음량을 1 증가.
			} 
		}
