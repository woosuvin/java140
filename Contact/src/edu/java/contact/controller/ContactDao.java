...
public interface ContactDao {
	 ...
	 * @return Contact 타입을 원소로 갖는 List 리턴.
	 */
	List<Contact> read();
	
	/**
	 * DB contacts 테이블에서 primary key로 연락처 정보를 검색.
	 * @param cid 검색 할 primary key.
	 * @return cid가 존재하면 Contact 타입 객체를 리턴. cid가 없으면 null 리턴.
	 */
	Contact read(int cid);
	
	/**
	 * primary key(cid)에 해당하는 연락처 정보를 수정(업데이트).
	 * @param contact 수정할 연락처의 PK(cid), 업데이트 할 이름, 전화번호, 이메일 정보를 저장한 객체.
