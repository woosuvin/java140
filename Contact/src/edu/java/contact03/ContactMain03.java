...
public class ContactMain03 {
	...
	public static void main(String[] args) {
		...
		ContactMain03 app = new ContactMain03(); // -> ContactMain03의 static이 아닌 필드와 메서드들을 사용하기 위해서는 객체가 생성되어 있어야 함.
	...
	private void insertNewContact() {
		...
		if(!dao.isMemoryAvailable()) { // 배열에 빈 공간이 없으면 (!true)
		...
		int result = dao.create(contact); // 컨트롤러 객체(dao)의 메서드를 호출(create)해서 연락처 저장 기능을 수행
	...
	private void selectAllContacts() {
		System.out.println("\n--- 연락처 전체 목록 ---");
		Contact[] contacts = dao.read(); // View(UI)는 Controller의 메서드를 호출해서 출력에 필요한 데이터를 리턴 받음.
