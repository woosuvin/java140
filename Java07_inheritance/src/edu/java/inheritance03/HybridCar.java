...
public class HybridCar extends Car{
	...
	public HybridCar(double fuel, double speed, int battery) {
		super(fuel, speed); // -> 상위 클래스에 기본 생성자가 없기 때문에, 반드시 argument를 갖는 super생성자를 명시적으로 호출해야 함.
		this.battery = battery; // -> 하위 클래스 필드를 초기화하는 코드는 super생성자 호출 이후에 작성.
	}
...
//				getSpeed(), getFuel(), this.battery);
	...
	// 메서드, 생성자 overloading:
	// parameter가 다를 때, 같은 이름으로 메서드, 생성자를 여러개 작성하는 것.
