...
public class InheritanceMain04 {
	...
	public static void main(String[] args) {
		...
		ch1.testParent(); // -> 실제 생성된 객체는 Child 타입이지만, 변수 선언이 Parent 타입으로 되어 있어서 Parent 타입에서 정의된 메서드들만 보이고 사용할 수 있음.
		((Child) ch1).testChild(); // -> parent타입 변수를 child타입으로 강제 타입 변환(casting)을 하면 Child 타입에서 정의된 메서드들도 보이고 사용할 수 있음.
		
		Parent ch2 = new AnotherChild();
		ch2.testParent();
		((AnotherChild) ch2).testAnotherChild();
//		((Child) ch2).testChild(); -> 문법 오류는 발생X, Child와 AnotherChild는 서로 casting 할 수 없는 타입이기 때문에 실행 중에 에러 발생.
		
		// instanceof 연산자 (변수 instanceof 클래스 이름)
		// 변수가 클래스 타입의 인스턴스이면 true, 그렇지 않으면 false 리턴해줌.
