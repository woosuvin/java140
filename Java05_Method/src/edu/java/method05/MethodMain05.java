...
public class MethodMain05 {
	...
	public static void main(String[] args) {
	    ...
	    // 가변 길이 argument에는 전달하는 값의 개수는 제한이 없음. argument를 전달하지 않아도 됨!
	    // 가변 길이 argument는 메서드 안에서는 !배열!처럼 취급함.
	    // (주의) 가변 길이 argument는 파라미터 중 가장 마지막 파라미터로 선언해야 함!
	    // (주의) 가변 길이 argument를 갖는 파라미터는 오직 1개만 선언할 수 있음.
	    
	    double result = calculate("+"); // 가변 길이 argument를 0개 전달한 경우.
	    System.out.println(result);
	    System.out.println(calculate("*"));
	    
	    result = calculate("+", 1, 2, 3, 4, 5); // 가변 길이 argument를 5개 전달한 경우.
	    System.out.println(result);
	    
	    result = calculate("*", 1, 2, 3, 4, 5);
	    System.out.println(result);
	    
	    double[] array = {1.0, 2.9}; // 가변 길이 argument일 때는 배열을 전달해도 됨.
	    System.out.println(calculate("+", array));
	    
	}
	
	/**
	 * argument op가 "+"인 경우에는, 모든 가변 argument들의 합계를 반환하고,
	 * argument op가 "*"인 경우에는, 모든 가변 argument들의 곱을 반환하는 메서드.
