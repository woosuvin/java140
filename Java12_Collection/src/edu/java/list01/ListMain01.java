 ...
 * 2. 값들을 저장하는 순서가 중요함. -> index를 가지고 있음. 
 * 3. 값들을 저장할 때마다 인덱스는 자동으로 증가함. -> 배열과는 다르게 저장 용량이 자동으로 증가.
...
public class ListMain01 {
	...
	public static void main(String[] args) {
		...
		System.out.println("리스트 크기: " + languages.size()); // -> 0
		...
		// 리스트에서 인덱스 위치의 원소를 리턴(검색): get(int index)
		System.out.println(languages.get(1)); // 인덱스는 0부터 시작.
		...
		System.out.println(languages); // -> 처음 Java 삭제
		
		// remove(int index): 리스트에서 인덱스 위치에 있는 원소를 삭제.
		languages.remove(0);
		System.out.println(languages); // -> SQL 삭제
		
		// 향상된 for구문(for - each): for(변수 선언 : 리스트) {...}
