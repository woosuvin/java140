...
public class FileMain08 {
	...
	public static void main(String[] args) {
		...
		// 1. Java11 이전 버전에서 문자 스트림을 사용하는 방법:
		//    FileReader, FileWriter는 권장하지 않음.
		try ( FileInputStream in = new FileInputStream(ansiFile); // 1. byte stream 생성
				InputStreamReader reader = new InputStreamReader(in, "EUC-KR"); // 2. 인코딩을 설정한 문자 스트림을 생성
				BufferedReader br = new BufferedReader(reader);) { // 3. 메모리 버퍼를 사용해서 읽기 속도를 빠르게 하기 위해 bufferedreader 생성.
	...
	System.out.println("\n-----------------------------");
		...
		// 2. Java11 버전부터 문자 스트림을 사용하는 방법:
		//    FileReader, FileWriter에서 인코딩 방식을 설정할 수 있게됨.
		try ( FileReader reader = new FileReader(utf8File, Charset.forName("UTF-8")); // 1. FileReader를 생성할 때 인코딩 방식을 설정.
				BufferedReader br = new BufferedReader(reader);) { // 2. 메모리 버퍼를 사용해서 읽기 속도롤 빠르게.
			
			while (true) {
//				int read = br.read(); // 한 글자씩 읽음.
//				if (read == -1) {
//					break;
//				}
//				System.out.print((char)read);
