...
public class FileMain09 {
	...
	public static void main(String[] args) {
		...
		// CWD(Current Working Directory, 현재 작업 디렉토리): 현재 프로그램이 실행되고 있는 위치.
		
		/*
		 * 절대 경로(absolute path):
		 * 	시스템의 루트(c:\, D:\, ...)부터 파일 또는 폴더가 있는 위치를 찾아가는 방식.
		 *  (예) E:\git\java140\Java13_File\data\encoding-ansi.txt
		 *  
		 * 상대 경로(relative path):
		 ...
		 *      (예) ..\Java11_MVC\src
		 * 
		 * 파일 구분자(file seperator):
		 *  상위 폴더와 하위 폴더, 폴더와 파일을 구분하기 위한 문자.
		 *  MS-Windows: \(backslach)
		 ...
		 */
		 ...
		 // backslach가 포함된 문자열
		 System.out.println("E:\\git\\java140\\Java13_File\\data\\encoding-ansi.txt");
		 ...
		 // -> getPath() 메서드: 파일 객체가 생성될 때 상대 경로 생성된 경우 getPath() 메서드는 상대 경로를 리턴.
		 ...
		 System.out.println(dataDir.getAbsolutePath()); // -> 절대경로
		 System.out.println("--------------------");
		 File[] files = dataDir.listFiles(); // 폴더의 하위 멤버들(폴더, 파일..)의 배열을 리턴.
		 for (File x : files) {
			 String fileName = x.getName(); // 파일 또는 폴더 이름.
			 String info = null; // 화면에 출력할 정보.
			 if (x.isFile()) { // 파일인 경우
				 info = fileName + "\t" + x.length() + " bytes";
		 ...
		 File newFolder = new File("data", "temp"); // .\data\temp 현재 디렉토리에 (상위 디렉토리, 하위 디렉토리)를 만들겠다.
